...
class BatchIdempotencyTest {
    @Test void transitionReplayReturnsPriorResultWithoutNewWrites(){
        ...
        when(keys.findByActorAndKey("alice","key-1")).thenReturn(Optional.of(IdempotencyRecord.builder().actor("alice").key("key-1").resourceType("BATCH").resourceId("db1").build()));
